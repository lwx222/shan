<template>
    <el-table :data="records" style="width: 100%">
      ...
      <el-table-column prop="memberPhone" label="memberPhone"  width="120"/>
      <el-table-column prop="productId" label="productId"  width="120"/>
      <el-table-column prop="productName" label="productName"  width="120"/>
      <el-table-column prop="sendTime" label="sendTime"  width="120"/>
      <el-table-column prop="subscribeTime" label="subscribeTime"  width="120"/>
...
export default defineComponent({
  ...
  methods: {
    ...
    getpage(current_a,size_a){
        ...
        axios({
            method: 'post',
            url: 'http://mall.qzimp.cn/mall/sms-flash-promotion-session/page',
            data:{
                current:current_a,
                size:size_a
            }
    ...
    Demandeupdate(){
      axios({
            method: 'post',
            url: 'http://mall.qzimp.cn/mall/sms-flash-promotion-session/edit',
            data:this.record
        }).then(function (response) {
    ...
    Demanderemove(id){
      axios({
            method: 'get',
            url: 'http://mall.qzimp.cn/mall/sms-flash-promotion-session/del/'+id,
        }).then(function (response) {
    ...
    Demandeadd(){
      axios({
            method: 'post',
            url: 'http://mall.qzimp.cn/mall/sms-flash-promotion-session/add',
            data:this.record
        }).then(function (response) {
          if (response.data.success) {
            ElMessage({ message: '操作成功.',type: 'success',})
